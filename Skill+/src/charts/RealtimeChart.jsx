...
function RealtimeChart({
  ...
  useEffect(() => {
    const currentValue = Math.round(data.datasets[0].data[data.datasets[0].data.length - 1]);
    const previousValue = Math.round(data.datasets[0].data[data.datasets[0].data.length - 2]);
    const diff = ((currentValue - previousValue) / previousValue) * 100;
  
    // Update the displayed current value
    chartValue.current.innerHTML = currentValue;
  
    // Set the deviation styling and value
    if (diff < 0) {
    ...
    <React.Fragment>
      <div className="px-5 py-3">
        <div className="flex items-start">
          <div className="text-3xl font-bold text-gray-800 dark:text-gray-100 mr-2 tabular-nums"><span ref={chartValue}>57</span></div>
